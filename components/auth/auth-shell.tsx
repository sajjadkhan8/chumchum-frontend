...
export function AuthWordmark({ light = false }: { light?: boolean }) {
  return (
    <Link href="/" className="inline-flex items-center gap-2.5" aria-label="ZingZing home">
      <span className={`grid size-8 place-items-center rounded-xl text-sm font-black ${light ? 'bg-white text-[#185c39]' : 'bg-[#185c39] text-white'}`}>
        Z
      </span>
      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] lg:grid-cols-[minmax(0,0.92fr)_minmax(480px,0.72fr)]">
        <section className="relative hidden overflow-hidden bg-[#173b2a] p-8 text-white lg:flex lg:flex-col xl:p-12">
          <div className="relative z-10 flex items-center justify-between">
            <AuthWordmark light />
            <Link href="/" className="inline-flex items-center gap-2 text-xs font-bold text-[#c9dace] transition hover:text-white">
          ...
          >
            ...
            <p className="mt-5 max-w-lg text-base leading-7 text-[#c9dace]">{description}</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="relative z-10 overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#214b36] shadow-2xl shadow-black/15"
          >
            <Image
            ...
            <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 bg-[#173b2a]/90 p-5 backdrop-blur-sm">
              <div>
                ...
                <p className="mt-1 text-[11px] text-[#c9dace]">Clear opportunities. Real local reach.</p>
              </div>
              <span className="hidden rounded-full bg-[#e6aa38] px-3 py-2 text-[11px] font-extrabold text-[#173b2a] xl:inline-flex">
                Food first
              </span>
            </div>
          </motion.div>

          <div className="relative z-10 mt-5 flex items-center justify-between text-[11px] font-semibold text-[#9fb9a8]">
